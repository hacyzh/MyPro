...
c-----
      call showinput(ndata,xmom,ev,z,m)
...
c-----
      ...
      subroutine showinput(ndata,xmom,ev,z,m)
c-----
...
c---- sorting the eigenvalues (principal stresses) into ascending order
   ...
   10 evp(i+1)=a
      index(i+1)=nin
    8 continue
c     write(LOT,'(a)') '  SORTED EIGENVALUES'
c     write(LOT,6) (evp(i),i=1,ndata)
...
c---- Minor double couple
    ...
    2 format(5e15.7)
     ...
     $'  SEISMIC MOMENTS OF ISOTROPIC PART, EMT1, EMT2, EMT3'
      ...
      if(test.lt.thresh) write(LOT,7) (z(j,k),j=1,3)
    6 continue
    2 format(5e15.7)
    7 format(5f11.7)
      return
...
c       write(6,*) ' ACOS(Z) = 0; Z = ',z
   ...
   25 del=90.
      return
   21 del=atan(y/x)/degrad+180.
      go to 28
   22 if(y)26,27,27
   27 del=atan(y/x)/degrad
      go to 28
   26 del=atan(y/x)/degrad+360.
   28 if(del.gt.360.) del=del-360.
...
c-----
      ...
      real*8 HMij(3,3)
      integer NR, NC
      parameter (NR=72, NC=72)
      character thePattern(NR)*80
...
c-----
      ...
      write(FID,'(a)')' Moment Tensor: (dyne-cm) Aki-Richards'
      write(FID,'(a)')'    Component   Value'
      write(FID,'(a,1pe9.2)')'       Mxx   ', Mij(1,1)
      write(FID,'(a,1pe9.2)')'       Mxy   ', Mij(1,1)
      write(FID,'(a,1pe9.2)')'       Mxz   ', Mij(1,3)
      write(FID,'(a,1pe9.2)')'       Myy   ', Mij(2,1)
...
c----
      ...
      integer nrows, ncols, i
           do i=1,nrows
                write(FID,'(a)')thePattern(i)(1:ncols)
           enddo
      return
      end
