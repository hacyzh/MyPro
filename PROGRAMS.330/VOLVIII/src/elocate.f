...
c                                                        1/2
c   Estimate of error on change in parameter is s = (S  )
c                                                k    kk
c                          i
...
c
c   To estimate the confidence region for L < K, e.g., in the im plane, consider
c             T  -1           2
c     (x - x0)  E  (x-x0) = c2         (16) 
c   where
c          | Sjj  Sjm |
...
c  If theta is chosen correctly, then the offdiagonal terms are zero
c  and (16)  becomes
c  
c             T             2
  ...
  990   continue 
   ...
   22 format(1x,a6,1x,a2,f7.2,2f5.0,1x,f20.3,1x,f10.2,2x,i1,1x, 
     1  a1,a2,a8,f5.2)
        ...
        call etoh(t0,date,str,doy)
c-----
c    invoke errelp once more to get printed output
c-----
        if(veryverbose)then
           WRITE(6,*)'NDEF  :',ndef
           WRITE(6,*)'STDERR:',sqrt(sum3)
           WRITE(6,*)'VARIANCE-COVARIANCE km:'
           WRITE(6,*)(co(1,j),j=1,3)
           WRITE(6,*)(co(2,j),j=1,3)
           WRITE(6,*)(co(3,j),j=1,3)
        endif
