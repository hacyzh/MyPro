/* changes
	19 NOV 2019 added TIMMIN TIMMAX  
        14 JAN 2020 to avoid confusion with the keywords CONcat and COLon three letters
              must be entered for format
*/
#include	<stdio.h>
...
struct arghdr fileidarg[] = {
	...
	{FILEID_FORMAT,  "FORMAT"  , CHDR, 0,   1, NO, "FOrmat EQUals|COLons|NONames",2},
	...
	{FILEID_LOCATION,"LOCATION", CHDR, 0,   1, NO, "LOcation UL|UC|UR|LL|LC|LR", 2},
        {FILEID_CONCAT,  "CONCAT"  , YHDR, 0,   1, NO, "CONCAT [ON|OFF] ", 3},
		...
		if(fileidarg[i].used > 0){
			...
			switch(fileidarg[i].id){
				...
				case FILEID_FORMAT:
					...
					else if(strncmp(instr,"EQU",3)==0)
						fileid_format = FILEID_EQUALS;
					else if(strncmp(instr,"NON",3)==0)
						fileid_format = FILEID_NONAMES;
					break;
...
/*
fprintf(stderr,"fileid_concat %d \n",fileid_concat);
*/
			if(fileid_concat == NO){
...
*/
				if(LCR < 0)
					gleft (xp + ht,   yp,ht,fileid_concatstr,0.0);
				else if(LCR == 0)
					gcent (xp        ,yp,ht,fileid_concatstr,0.0);
				else if(LCR > 0)
					gright(xp -ht -ht,yp,ht,fileid_concatstr,0.0);
