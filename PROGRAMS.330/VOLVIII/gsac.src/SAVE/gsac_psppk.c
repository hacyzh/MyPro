...
#define PSPPK_SMOOTH 20
#define PSPPK_SMOOTH_PASS 21
...
*/
		...
		case 'L':        /* give cursor location and amplitude
			...
			gclip("off", x0, y0, x0+xlen, y0+ylen);
			/* note:  ux is the position in the window. getfreq uses this position
			...
			*/
			  getfreq(ux,k,ns,ne,numpsppkframe,lpos,x0);
			gclip("on", x0, yl , x0+xlen, yh);
			  break;
...
int m;
	x = (float *)realloc(y,np*sizeof(float));
	for(m=0;m < np ; m++)
		x[m] = y[m];
