/* Changes:
	20 JUL 2007 - place the time of the maximum of the cross-correlation
		into T9
	16 OCT 2017 - ensure that the cross-correlation correctly gives the
		difference in distances
	31 DEC 2017 - added LCALDA == 1 to the recomputation of distances
*/
...
printf("CORRELATE MASTER %d DOTWO %s DOREVERSE %s LENGTH %s NUMBER %s \n",
	...
	distx = sacdata[corr_master].sachdr.rhdr[H_DIST];
	...
	for ( k=0 ; k < ntrc ; k ++){
		...
		*/
		...
		if(stla != -12345. && stlo != -12345. 
			...
			sacdata[k].sachdr.rhdr[H_DIST] = dist - distx;
		}
