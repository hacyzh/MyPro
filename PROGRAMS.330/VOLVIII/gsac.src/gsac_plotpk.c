...
extern void gsac_show_plotpk(float x0, float y0, float xlen, float ylen, int ns, int ne, float dy, int ntrc, double twin,int numperframe, int pabsolute, float *uy, float *ly, int setup, int ylimctrl, float ylimlow , float ylimhigh, int overlay);
...
extern int inside(float xv, float yv, float xlb, 
...
void gsac_exec_plotpk(void)
{
	...
	for ( kkk=0 ; kkk < ntrc && iret == NEXT ; kkk+=numperframe){
		...
		gsac_show_plotpk( x0,  y0,  xlen,  ylen,  kkk,  
				MIN(kkk+numperframe,ntrc),  dy, ntrc, 
...
float cx_X, twin_X;	/* for use with the X X to define window */
	...
	switch(switchchar){
		case '~':
			...
			gsac_show_plotpk(x0,y0,xlen,ylen,ns,ne,dy,ntrc, 
				twin, numperframe, pkabsolute,&yh,&yl,
				NO,YLIM_OFF, -1, +1, NO);
...
void traceinfo(float cx,float cy,float x0,float y0, float dy, float xlen,float ylen,int ns, int ne, int ntrc, int *k, float *ux, float *uy , int numperframe)
{
	/* 31 JUL 2015 corrected erris where *uy = urx was not correct */
	float uxv, uyv;
	int kv;
	int kk;
	...
	for(i=0 ; i < nmp ; i++){
		...
		if(inside(cx,cy,lx,ly,hx,hy)){
			/* 10 OCT 2012 this block of code was just above the previous statement */
				...
				if(last_h > 0){
					pickundo = YES;
					/* when undoing the pick, the tp has no meaning, so use dummy */
					tp = 0.0;
					...
					gclip("off", x0, y0, x0+xlen, y0+ylen);
					clearregion(0.0,1.0,10.0,8.0);
					gsac_show_plotpk(x0,y0,xlen,ylen,ns,ne,dy,
